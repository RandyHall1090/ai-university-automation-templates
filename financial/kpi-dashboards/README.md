...
Reads `financials.csv` (`month,cash_balance,expenses,budget`) and `receivables.csv` (`invoice_id,amount,due_date,paid`). Computes: current cash balance, average monthly burn (expenses over the trailing 3 months), runway in months (cash ÷ burn), AR aging buckets (current / 1–30 / 31–60 / 61–90 / 90+ days past due, unpaid invoices only), and budget vs. actual variance per month. Outputs `financial-kpi-report.md` and `.json`.
